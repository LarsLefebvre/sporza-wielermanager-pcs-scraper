...
def get_top_10_riders_from_race(race_url):
    """
    Fetches the top 10 riders for a specific race.
    """
    try:
        ...
        # Extract top 10 riders' names
        top_10_riders = [rider['rider_name'] for rider in riders[:10]]
        return top_10_riders
    except Exception as e:
        print(f"Error fetching top 10 for {race_url}: {e}")
        return []

# Dictionary to track appearances of each rider in the top 10
rider_appearance_counter = Counter()

# Loop through all races and collect the top 10 riders
for race_url in races:
    print(f"Processing race: {race_url}")
    top_10_riders = get_top_10_riders_from_race(race_url)
    
    # Update the counter with riders who appeared in the top 10
    rider_appearance_counter.update(top_10_riders)
    
    # Add a small delay to avoid overwhelming the server
    time.sleep(1)

# Now we have the count of appearances for each rider in the top 10 across all races
# Sort the riders by their appearances and print the top 10 (or any number)
top_riders = rider_appearance_counter.most_common()

# Print the top riders with the most appearances
print("\nTop Riders in the Top 10 across the races included in Sporza Wielermanager since 2022:")
for rank, (rider_name, appearances) in enumerate(top_riders[:10], 1):  # Display top 10
